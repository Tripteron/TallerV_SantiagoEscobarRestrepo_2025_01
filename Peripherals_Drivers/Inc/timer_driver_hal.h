...
typedef struct
{
	...
	uint8_t TIMx_InterruptEnable; // // Habilita o deshabilita el uso de interrupciones para el temporizador
} Timer_BasicConfig_t;
...
#define IS_TIMER_PRESC(VALUE) (((VALUE) > 0) && ((VALUE) <= 0xFFFF))
...
//Para evitar tener que copiar los callback repetidamente

//void Timer_Callback(uint8_t timerID);
